...
- **Generated media**, billed from credits (image ~1¢ to ~20¢, music ~10¢ per track, depending on the model; video: exact prices per length from generate-video called without `duration`). An OpenAI **API key** generates images on the user's own OpenAI account at no VibeKit charge; every other account (no key, an Anthropic key, a ChatGPT sign-in) generates images on VibeKit credits. The image and video models are picked in the AI model drawer's Images and Video tabs, newest by default. Music and video always bill credits.
